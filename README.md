...
- Маркирует сетевые соединения, используя заданный при помощи API уровень доступа (см.
конфигурирование)
...
- ``` mangle / PREROTING ``` (маркирование трафика)
- ``` nat / PREROUTING ``` (DNAT)
...
# например, local-domains
    ...
    - mode:
# марки,  заданные для режиме - может быть несколько
        marks: [0]
...
ndnproxy:
    ...
    - instance:
# TCP порт, на котором случает прокси
        tcp-port: 10053

# UDP порт, на котором случает прокси
        udp-port: 10053
